...
try {
    $sql_totals = "SELECT COUNT(lot_number) as total_lots, SUM(total_bags) as total_bags, SUM(total_weight_kg) as total_weight_kg FROM areca_inventory_history WHERE " . $whereClause;
    ...
    $summaryByType = [];
    $sql_lots = "SELECT lot_number, date_received, total_bags, total_weight_kg FROM areca_inventory_history WHERE " . $whereClause . " ORDER BY date_received DESC";
