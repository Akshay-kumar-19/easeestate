...
error_log("Received JSON Data: " . print_r($data, true)); // Debugging

$action_type = $data['action_type'] ?? 'batch_update'; // Default to batch if not specified

if ($action_type === 'individual_update') {
    // Handle Individual Worker Update
    $worker_id = $data['worker_id'] ?? null;
    $lead_id = $data['lead_id'] ?? null;
    $present = isset($data['present']) ? intval($data['present']) : null; // Ensure integer
    ...
    // Handle Batch Attendance Update (Submit Button) - Existing Logic
    ...
    // Check if attendance already submitted for this lead and date (for batch submit only)
