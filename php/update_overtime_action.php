...
// Enable error logging (already present, ensure it's configured)
...
// Check if overtime already submitted for this lead and date (You can adapt this logic as needed)
...
if ($overtimeCount > 0) {
    ...
    exit; // Stop processing further updates
}
...
foreach ($overtime_entries as $entry) {
    ...
    $job_id = $entry['job_id'];
    $overtime_hours = floatval($entry['overtime_hours']); // Convert to float
    ...
    $sql = "INSERT INTO overtime (worker_id, lead_id, job_id, user_id, date, overtime_hours)
            VALUES (?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE overtime_hours = VALUES(overtime_hours), job_id = VALUES(job_id)"; // Update overtime_hours and job_id on duplicate
    ...
    $stmt->bind_param("iiiisd", $worker_id, $lead_id, $job_id, $user_id, $date, $overtime_hours); // Note the 'd' for decimal/double
