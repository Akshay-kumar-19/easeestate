...
try {
    $stmt = $conn->prepare("SELECT w.worker_id, w.worker_name, ll.lead_name, ll.lead_id, j.per_kg_rate, j.daily_wage
                                ...
                                WHERE w.user_id = ? AND a.job_id = ? AND a.date = CURDATE() AND a.present = 1"); // Modified query with attendance join
    $stmt->bind_param("ii", $user_id, $job_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $workers = $result->fetch_all(MYSQLI_ASSOC);

    // Fetch per_kg_rate and daily_wage even if no workers are found
