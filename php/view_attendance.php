...
// Fetch Labour Leads for dropdown
...
// Function to get week numbers for a given month and year
function getWeekNumbersForMonth($month, $year) {
    $weekNumbers = [];
    $dateTime = new DateTime();
    $dateTime->setDate($year, $month, 1); // Set to the first day of the month

    $monthToCheck = $dateTime->format('n'); // Month number to compare against

    while ($dateTime->format('n') == $monthToCheck) {
        $weekNumber = (int)$dateTime->format('W'); // Get week number, cast to integer to remove leading zero
        if (!in_array($weekNumber, $weekNumbers)) { // Avoid duplicate week numbers if a week spans months
            $weekNumbers[] = $weekNumber;
        }
        $dateTime->modify('+1 week'); // Move to the next week
    }
    sort($weekNumbers); // Sort week numbers in ascending order
    return $weekNumbers;
}

$currentYear = date('Y'); // Use current year as default
