<?php
# fertilizer_assign_action.php - Backend for Assigning Fertilizer with Team Lead (Required)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $team_lead_id = $_POST['team_lead_id']; // Changed variable name - get team_lead_id from form
    ...
    if (empty($date_used) || empty($team_lead_id) || empty($fertilizer_name) || empty($quantity_used) || !is_numeric($quantity_used) || $quantity_used <= 0 || empty($unit) || empty($field_location)) {
        echo json_encode(['status' => 'error', 'message' => 'Invalid input data. Please check all required fields.']); // Updated message
        exit();
    }
    ...
    try {
        ...
        // --- 0. Check Current Stock BEFORE assignment ---
        ...
        if ($result_check_stock->num_rows === 0) {
            throw new Exception("Fertilizer name not found in inventory."); // Should not happen if dropdowns are used correctly
        }
        ...
        // 1. Insert into fertilizer_usage_history table - includes lead_id (still using 'lead_id' in DB)
        ...
        $stmt_insert_usage->bind_param("sisdss", $date_used, $team_lead_id, $fertilizer_name, $quantity_used, $unit, $field_location); // Using $team_lead_id
        ...
        if (!$stmt_insert_usage->execute()) {
            ...
            error_log("Successfully inserted into fertilizer_usage_history for fertilizer: " . $fertilizer_name . ", team lead: " . $team_lead_id); // Updated log message - team lead
        }

        // 2. Update fertilizer_inventory table - decrease quantity
        ...
        $conn->commit();
        echo json_encode(['status' => 'success', 'message' => 'Fertilizer assigned successfully.', 'reload' => true]); // Added reload for page refresh
