...
try {
    ...
    $total_unripe_kg = 0;
    $total_total_kg = 0;
    $total_pepper_kg = 0; // Initialize total pepper kg
    $total_areca_kone = 0; // Initialize total areca kone
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $crops_data[] = [
                ...
                'job_name' => $row['job_name'], // Include job_name in data
            ];
            if ($row['job_name'] == 'cofffee plucking') { // Accumulate coffee totals
                $total_ripe_kg += $row['ripe_kg'];
                $total_unripe_kg += $row['unripe_kg'];
                $total_total_kg += $row['total_kg'];
            } else if ($row['job_name'] == 'pepper plucking') { // Accumulate pepper total
                $total_pepper_kg += $row['total_kg'];
            } else if ($row['job_name'] == 'arreca plucking') { // Accumulate areca total
