...
try {
    // 1. Calculate overall totals with filtering
    ...
    // 2. Calculate summary by coffee type with filtering - Modified to get separate details for each type
    ...
    if (!empty($params)) {
        if ($summaryType == 'yearly') {
            $stmt_by_type->bind_param("s", $params[0]); // 's' for year
        } else if ($summaryType == 'monthly') {
            $stmt_by_type->bind_param("ss", $params[0], $params[1]); // 'ss' for year and month
        }
    }
    ...
    $summaryByType = [];
    $parchmentSummary = ['total_bags' => 0, 'total_weight_kg' => 0]; // Initialize for Parchment
    $cherrySummary = ['total_bags' => 0, 'total_weight_kg' => 0]; // Initialize for Cherry


    while ($row = $result_by_type->fetch_assoc()) {
        $summaryByType[] = $row; // Keep the summary by type array for table display
