...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    // Moisture level is NOT taken from POST

    // Validate inputs (add more validation as needed)
    if (empty($lot_number) || empty($date_received) || empty($total_bags) || empty($total_weight_kg)) { // Moisture level removed from required fields
    ...
    try {
        ...
        // 1. Insert into areca_inventory (current inventory) - No moisture_level
        ...
        $stmt_current->bind_param("isddi", $lot_number, $date_received, $total_bags, $total_weight_kg, $user_id); // No moisture_level in bind_param
        if (!$stmt_current->execute()) {
            throw new Exception("Execute failed (current inventory): " . $stmt_current->error);
        }
        $stmt_current->close();

        // 2. Insert into areca_inventory_history (history log) - No moisture_level
        ...
        $stmt_history->bind_param("isddi", $lot_number, $date_received, $total_bags, $total_weight_kg, $user_id); // No moisture_level in bind_param
