...
function calculateWeeklySalaryForWorker($conn, $worker_id, $lead_id, $week_start_date, $week_end_date) {
    ...
    if ($worker_row = $worker_result->fetch_assoc()) {
        ...
        return null; // Worker not found, or error
    }

    // Fetch attendance for the week, including daily_wage and per_kg_rate
    ...
    $daily_wage_rate = 0;
    $per_kg_rate_value = 0;
    $is_per_kg_job = false; // Flag to check if it's a per kg job

    while ($attendance_row = $attendance_result->fetch_assoc()) {
        $present_days++;
        // Assuming job details are consistent for the week, using details from the first attendance record
        if ($daily_wage_rate == 0 && $per_kg_rate_value == 0) {
            ...
            if ($per_kg_rate_value > 0) {
                $is_per_kg_job = true; // Set flag if per kg rate is found
            }
        }
    }
    $salary_details['present_days'] = $present_days;

    // Calculate salary based on job type (Daily Wage OR Per KG)
    if ($is_per_kg_job) {
        $salary_details['daily_wage_salary'] = 0; // Do not calculate daily wage if per kg job
        // Fetch crops plucked data for per_kg salary
        ...
        // If not a per kg job, calculate daily wage salary
        $salary_details['daily_wage_salary'] = $present_days * $daily_wage_rate;
        $salary_details['per_kg_salary'] = 0; // Ensure per_kg_salary is 0 for daily wage jobs
    }


    // Fetch total overtime hours and calculate overtime salary (Overtime calculation remains the same)
    ...
    if ($overtime_row = $overtime_result->fetch_assoc()) {
        ...
        // Fetch overtime hourly rate from jobs table
        ...
        if ($job_id_from_overtime > 0) { // Use job_id from overtime if available, otherwise fallback to attendance job_id
         ...
         if ($overtime_hourly_rate == 0) { // Fallback to job from attendance if no overtime job and no overtime rate found yet
...
function getWeeklySalariesByLead($conn, $lead_id, $week_start_date, $week_end_date) {
    ...
    while ($worker_row = $workers_result->fetch_assoc()) {
        ...
        if ($worker_salary) { // Check if salary calculation was successful for the worker
...
function outputCsv($all_salaries_data) {
    ...
    foreach ($all_salaries_data as $lead_data) {
        if (isset($lead_data['worker_salaries'])) {
            ...
            // Output Lead Totals row
            fputcsv($output, [
                $lead_data['lead_name'] . ' Totals',
                '', // Worker Name - blank for totals
             ...
             // Empty row for separation between leads
            fputcsv($output, []);
        }
    }
...
if (isset($_GET['week_start']) && isset($_GET['week_end'])) {
    ...
    if (isset($_GET['report_format']) && $_GET['report_format'] === 'csv') {
        ...
        exit(); // Stop further execution after CSV output
    } else {
        // Default to JSON output if report_format is not 'csv' (for displaying on page)
