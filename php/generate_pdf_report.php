...
require_once 'C:\xampp\htdocs\easeestate\tcpdf\tcpdf\tcpdf.php';
 // Include TCPDF library (adjust path if needed)
...
// Get week start and end dates from GET parameters
...
// Set font
$pdf->SetFont('dejavusans', '', 10, '', true); // For UTF-8 support
...
foreach ($all_salaries_data as $lead_data) {
    if (!empty($lead_data['worker_salaries'])) {
        ...
        $pdf->Cell(35, 7, 'Total Salary', 1, 1, 'R', 1); // End of row
        ...
        foreach ($lead_data['worker_salaries'] as $worker_salary) {
            ...
            $pdf->Cell(35, 6, '₹' . number_format($worker_salary['total_salary'], 2), 1, 1, 'R'); // End of row
        }

        // Total row for each lead
        $pdf->SetFont('dejavusans', 'B', 9, '', true);
        ...
        $pdf->Cell(35, 7, '₹' . number_format($lead_data['totals']['grand_total_salary'], 2), 1, 1, 'R'); // End of row
        $pdf->Ln(5); // Extra space after each lead table

        $grand_total_all_leads_pdf += $lead_data['totals']['grand_total_salary'];
    }
}

// Grand total for all leads
$pdf->SetFontSize(14);
