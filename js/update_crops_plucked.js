document.addEventListener("DOMContentLoaded", function () {
    ...
                    // Add event listeners for real-time calculation for coffee
                    if (jobId == coffee_job_id) {
    ...
                // Update Total Plucked (KG) header based on crop type
                const totalPluckedHeader = document.querySelector('#workersTable th[data-crop-type="coffee"]:nth-child(5)'); // Select "Total Plucked (KG)" header for coffee
                if (cropType == coffee_job_id) {
                    totalPluckedHeader.textContent = "Total Plucked (KG)"; // Default text for coffee
                } else if (cropType == pepper_job_id) {
                    totalPluckedHeader.textContent = "Total Plucked (KG)"; // Or keep it "Total Plucked (KG)" for pepper as well, if applicable
                } else if (cropType == areca_job_id) {
                    totalPluckedHeader.textContent = "Total Count (Kone)"; // Change header for Areca to "Total Kone" or similar
                } else {
                    totalPluckedHeader.textContent = "Total Plucked"; // Default if no crop is selected
                }
            }
    ...
                perKgBtn.classList.add('active');
                dailyWageBtn.classList.remove('active');
                if (workersSection.style.display === 'block') { // If worker table is already displayed, re-populate to update button styling
                    fetchWorkersForCrop(selectedCropType); // Re-fetch workers to re-apply button styling in populateWorkerTable
                }
            });
    ...
                            isValid = false;
                            return; // Stop processing current row and further rows
                        }
                        totalKg = parseFloat(row.querySelector('.total-plucked-kg').textContent) || 0; // Get calculated total KG
    ...
                        ripe_kg: ripeKg,
                        unripe_kg: unripeKg,
                        total_kg: totalKg, // Include totalKg in pluckedData
                        kone_count: koneCount,
                    });
                });
    
                if (!isValid) { // If validation failed for any row, stop submission
                    return;
                }
                // --- END OF VALIDATION ---
    ...
            // Set initial active button on page load (default to Per KG)
            if (salaryCalculationType === 'per_kg') {
                perKgBtn.classList.add('active');
                dailyWageBtn.classList.remove('active');
            }
