document.addEventListener("DOMContentLoaded", function () {
    ...
    const summaryButtons = document.querySelectorAll('.summary-btn'); // Select all summary buttons

    // Function to handle button active state
    function setActiveButton(button) {
        summaryButtons.forEach(btn => btn.classList.remove('active')); // Remove active class from all buttons
    ...
    // Initial summary load (Yearly by default)
    fetchCropSummaryData('yearly');
    ...
    function displayCropSummaryData(summaryData, summaryType) {
        cropSummaryTableContainer.innerHTML = ''; // Clear previous table
        ...
        summaryData.forEach(record => {
            ...
            // Determine the time period label based on summaryType
            let periodLabel = '';
            if (summaryType === 'yearly') {
                periodLabel = record.year;
            } else if (summaryType === 'monthly') {
                periodLabel = record.month; // Assuming your PHP returns month in a readable format
            ...
            row.insertCell().textContent = record.coffee_ripe_kg || '0.00'; // Use '0.00' if null
            ...
            row.insertCell().textContent = record.areca_total_kone || '0'; // Use '0' for kone if null
        });
