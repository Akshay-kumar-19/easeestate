document.addEventListener("DOMContentLoaded", function () {
    ...
    const jobDropdownMain = document.getElementById("main_job_id"); // Get "Select Job (For All)" dropdown
    ...
    function fetchWorkers(leadId) {
        clearWorkerTable(); // Clear previous data
        if (!leadId) {
            return; // Don't fetch if lead is not selected
        }

        fetch(`fetch_workers.php?lead_id=${leadId}`) // Fetch workers for lead
            .then(response => {
                if (!response.ok) {
    ...
    // Function to generate job options for dropdowns (using jobs data fetched in PHP)
    ...
    // Event listener for Main Job dropdown change ("Select Job (For All)")
    jobDropdownMain.addEventListener("change", function () {
        const selectedJobId = this.value;
        document.querySelectorAll(".job-dropdown").forEach(dropdown => {
            dropdown.value = selectedJobId; // Set each worker's job dropdown to the selected job
        });
    });
    ...
    // Toggle Present/Absent per worker
    ...
    submitBtn.addEventListener("click", function () {
        ...
        document.querySelectorAll("#workersTable tbody tr").forEach(row => {
            ...
            let jobId = row.querySelector(".job-dropdown").value; // Get selected job ID for each worker
            attendanceData.push({ worker_id: workerId, present: status, job_id: jobId }); // Include job_id
        });

        if (attendanceData.length === 0) {
            alert("No workers to submit attendance for. Please select a valid Labour Lead.");
            return; // Exit if no worker data
        }
        ...
        fetch("update_attendance_action.php", {
            ...
            body: JSON.stringify({
                ...
                attendance: attendanceData // attendanceData now includes job_id
            })
        })
            .then(response => {
                if (!response.ok) {
